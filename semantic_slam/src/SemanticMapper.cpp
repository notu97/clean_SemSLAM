...
SemanticMapper::~SemanticMapper(){
    bool save_result;
    pnh_.getParam("save_result", save_result);
    ...
    pnh_.getParam("save_root", save_root);
    std::cout << "save_root: " << save_root << std::endl;

    int seq_id;
    pnh_.getParam("seq_id", seq_id);

    // save objects
    std::cout << "saving objects..." << std::endl;
    std::string obj_file_dir = save_root + std::string(3 - std::to_string(seq_id).length(), '0') + std::to_string(seq_id) + "/objects.txt";
    std::cout << obj_file_dir << std::endl;
    std::ofstream obj_file(obj_file_dir);

    if (obj_file.is_open()){
        for (size_t i = 0; i < estimated_objects_.size(); ++i){
            obj_file << "object_id: " << estimated_objects_[i]->id() << "\n";
            obj_file << "object_name: " << estimated_objects_[i]->obj_name() << "\n";
            obj_file << "is_bad: " << estimated_objects_[i]->bad() << "\n";
            obj_file << "first_frame: " << estimated_objects_[i]->first_seen() << " " << estimated_objects_[i]->getMeasurements()[0].stamp << "\n";
            obj_file << "last_frame: " << estimated_objects_[i]->last_seen() << " " << estimated_objects_[i]->getMeasurements().back().stamp << "\n";
            
            obj_file << "quaternion: " ;
            for (size_t j = 0; j < 4; ++j){
                obj_file << estimated_objects_[i]->pose().rotation_data()[j] << " ";
            }
            
            obj_file << "\ntranslation: ";
            for (size_t j = 0; j < 3; ++j){
                obj_file << estimated_objects_[i]->pose().translation_data()[j] << " ";
            }
            
            obj_file << "\ncoefficients: ";
            Eigen::VectorXd coefficients = estimated_objects_[i]->getCoefficients();
            for (size_t j = 0; j < coefficients.size(); ++j) {
                obj_file << coefficients(j) << " ";
            }

            // obj_file << "\nmsmt_global_ids: ";
            // aligned_vector <ObjectMeasurement> obj_msmts = estimated_objects_[i]->getMeasurements();
            // for (size_t j = 0; j < obj_msmts.size(); ++j) {
            //     obj_file << obj_msmts[j].global_msmt_id << " ";
            // }

            obj_file << "\nmsmt_track_ids: ";
            aligned_vector <ObjectMeasurement> obj_msmts = estimated_objects_[i]->getMeasurements();
            for (size_t j = 0; j < obj_msmts.size(); ++j) {
                obj_file << obj_msmts[j].track_id << " ";
            }

            obj_file << "\nbbox_firstframe: ";
            obj_file << obj_msmts[0].bbox.xmin << " " << obj_msmts[0].bbox.ymin << " " 
                     << obj_msmts[0].bbox.xmax << " " << obj_msmts[0].bbox.ymax;

            obj_file << "\nkeypoints_gobal: \n";
            for (size_t k=0; k < estimated_objects_[i]->numKeypoints(); ++k){
                for (size_t j=0; j < 3; ++j){
                    obj_file << estimated_objects_[i]->keypoints()[k]->position()[j] << " ";
                }
                obj_file << "\n";
            }

            // obj_file << "keypoints_local: \n";
            // for (size_t k=0; k < estimated_objects_[i]->numKeypoints(); ++k){
            //     for (size_t j=0; j < 3; ++j){
            //         obj_file << estimated_objects_[i]->keypoints()[k]->local_position()[j] << " ";
            //     }
            //     obj_file << "\n";
            // }
    ...
    // std::string cam_pose_file_dir = "/home/hanwen/code/semanticSLAM/cam_poses.txt";
    std::string cam_pose_file_dir = save_root + std::string(3 - std::to_string(seq_id).length(), '0') + std::to_string(seq_id) + "/poses.txt";
    std::ofstream cam_pose_file(cam_pose_file_dir);

    if (cam_pose_file.is_open()){
        for (size_t i=0; i<keyframes_.size(); ++i){
            cam_pose_file << "time_stamp: " << keyframes_[i]->time() << "\n";

            cam_pose_file << "quaternion: " ;
            for (size_t j=0; j < 4; ++j){
                cam_pose_file << keyframes_[i]->pose().rotation_data()[j] << " ";
            }
            
            cam_pose_file << "\ntranslation: ";
            for (size_t j=0; j < 3; ++j){
                cam_pose_file << keyframes_[i]->pose().translation_data()[j] << " ";
            }
    ...
    // save 2D object detection
    std::cout << "saving 2D object detections..." << std::endl;
    // std::string detection_2d_file_dir = "/home/hanwen/code/semanticSLAM/2d_detection.txt";
    std::string detection_2d_file_dir = save_root + std::string(3 - std::to_string(seq_id).length(), '0') + std::to_string(seq_id) + "/2d_detection.txt";
    std::ofstream detection_2d_file(detection_2d_file_dir);

    if (detection_2d_file.is_open()){
        for (size_t i=0; i<keyframes_.size(); ++i){
            if (keyframes_[i]->measurements().size() == 0) continue;
            detection_2d_file << "frame_time_stamp: " << keyframes_[i]->time() << "\n";
            detection_2d_file << "bounding boxes: \n";
            for (size_t j=0; j<keyframes_[i]->measurements().size(); ++j){
                // detection_2d_file << "bbox_time_step: " << keyframes_[i]->measurements()[j].stamp << "\n";
                detection_2d_file << "bbox_obj_name: " << keyframes_[i]->measurements()[j].obj_name << "\n";
                detection_2d_file << keyframes_[i]->measurements()[j].bbox.xmin << " ";
                detection_2d_file << keyframes_[i]->measurements()[j].bbox.ymin << " ";
                detection_2d_file << keyframes_[i]->measurements()[j].bbox.xmax << " ";
                detection_2d_file << keyframes_[i]->measurements()[j].bbox.xmin << "\n";
            }
            detection_2d_file << "\n";
        }
...
SemanticMapper::processMessagesUpdateObjectsThread()
{
    while (ros::ok() && running_) {
        ...
        if (did_anything) {
            ...
            // std::cout << typeid(presenters_).name() << std::endl;
            for (auto& p : presenters_){
                // std::cout << "test\n";
                // std::cout << typeid(p).name() << std::endl;
                p->present(keyframes_, estimated_objects_);
            }
            // std::cout << "number of keyframes: " << keyframes_.size() << std::endl;    
...
SemanticMapper::loadParameters()
{
    if (!pnh_.getParam("keypoint_activation_threshold",
        ...
        !pnh_.getParam("include_object_features",
                       include_object_features_) ||
        !pnh_.getParam("use_hungarian", use_hungarian_) ||
        !pnh_.getParam("detect_loop_closure", detect_loop_closure_) ||
    ...
    if (frame->measurements().size() > 0) {
        ...
        for (size_t j = 0; j < estimated_objects_.size(); ++j) {
            if (visible[j]) {
                ...
                avg_objz += estimated_objects_[j]->pose().translation()[2];
        ...
        Eigen::MatrixXd covDets =
          Eigen::MatrixXd::Zero(measurement_index.size(), n_visible);

        // std::cout << "mahals shape: " << measurement_index.size() << "x" << n_visible << std::endl;
        
        // std::cout << "before mahals computation" << std::endl;
        for (size_t i = 0; i < measurement_index.size(); ++i) {
            for (size_t j = 0; j < n_visible; ++j) {
                ...
                // double covDet;
                // // std::cout << "cov Det before: " << covDet << std::endl;
                // mahals(i, j) = estimated_objects_[object_index[j]]
                //                  ->computeMahalanobisAndDet(
                //                    frame->measurements()[measurement_index[i]], covDet);
                // // std::cout << "cov Det: " << covDet << std::endl;
                // covDets(i, j) = covDet;
                ...
                if (use_hungarian_) {
                    ...
                    // std::cout << "using hungarian" << std::endl;
                }
                else {
                    double covDet;
                    // std::cout << "cov Det before: " << covDet << std::endl;
                    mahals(i, j) = estimated_objects_[object_index[j]]
                                    ->computeMahalanobisAndDet(
                                    frame->measurements()[measurement_index[i]], covDet, avg_objz);
                    // std::cout << "cov Det: " << covDet << std::endl;
                    covDets(i, j) = covDet;
                }
        ...
        // std::cout << "after mahals computation" << std::endl;
        // std::cout << "before weight computation" << std::endl;
        ...
        if (use_hungarian_) {
            ...
            weights = MLDataAssociator(params_).computeConstraintWeights(mahals);
            // std::cout << "hungarian weights:\n" << weights << std::endl;
        }
        else {
            /*********************************** DEBUG ******************************************/
            // std::cout << "mahalanobis distance:\n" << mahals << std::endl;
            // std::cout << "covariance determinent:\n" << covDets << std::endl;
            
            // Eigen::MatrixXd hungarian_weights = MLDataAssociator(params_).computeConstraintWeights(mahals);
            // Eigen::MatrixXd hungarian_sum = hungarian_weights.rowwise().sum();
            
            // for (size_t i = 0; i < hungarian_sum.rows(); ++i) {
            //     if (hungarian_sum(i, 0) == 0) {
            //         std::cout << "zero row sum!" << std::endl;
            //         break;
            //     }
            // }
            
            bool valid = true;
            // if (std::max(mahals.rows(), mahals.cols()) > 24) {
            //     std::cout << "large mahals matrix with size: (" << mahals.rows() << ", " << mahals.cols() << ")" << std::endl;
            // }
            weights = ProbDataAssociator(params_).computeWeightsWithPermanent(mahals, covDets, 24, valid);
            ...
            Eigen::MatrixXd hungarian_weights = MLDataAssociator(params_).computeConstraintWeights(mahals);
            if (!valid) {
                std::cout << "probabilistic association computation failed!!!" << std::endl;
                weights = hungarian_weights;
            }

            /********************************** DEBUG: print difference between probabilistic and hungarian *************************************/
            double dist = (weights - hungarian_weights).array().abs().sum();
            double thre = -0.1;
            if (dist > thre) {
                std::cout << "difference is larger than the thre: " << dist << std::endl;
                std::cout << "hungarian weights:\n" << hungarian_weights << std::endl;
                std::cout << "weights:\n" << weights << std::endl;
            }

            /**************************************** save results for association visualization **********************************************/
            // if (dist > thre) {
            //     std::string visu_save_root = "/home/hanwen/code/object_slam/semanticSLAM/estim_result/visu/sem_assoc/022/";
            //     int frame_idx = frame->index();
            //     std::string visu_save_dir = visu_save_root + std::to_string(frame_idx) + ".txt";

            //     std::ofstream visu_save_file(visu_save_dir);
            //     // save time
            //     visu_save_file << "time: " << frame->time() << "\n";
            //     visu_save_file << "imu_quaternion: ";
            //     for (size_t j = 0; j < 3; ++j) {
            //         visu_save_file << frame->pose().rotation().vec()[j] << " ";
            //     }
            //     visu_save_file << frame->pose().rotation().w() << std::endl;

            //     visu_save_file << "imu_translation: ";
            //     for (size_t j = 0; j < 3; ++j) {
            //         visu_save_file << frame->pose().translation()[j] << " ";
            //     }
            //     visu_save_file << "\n\n";

            //     // save all measurements
            //     visu_save_file << "### Measurements ###\n";
            //     for (size_t i = 0; i < frame->measurements().size(); ++i) {
            //         ObjectMeasurement obj_meas = frame->measurements()[i];

            //         visu_save_file << "meas_track_id: " << obj_meas.track_id << std::endl;
            //         visu_save_file << "meas_obj_name: " << obj_meas.obj_name << std::endl;

            //         visu_save_file << "2D_bbox: "
            //             << obj_meas.bbox.xmin << " "
            //             << obj_meas.bbox.ymin << " "
            //             << obj_meas.bbox.xmax << " "
            //             << obj_meas.bbox.ymax << "\n";
                    
            //         visu_save_file << "2D_keypoints: \n";

            //         for (size_t j = 0; j < obj_meas.keypoint_measurements.size(); ++j) {
            //             visu_save_file << obj_meas.keypoint_measurements[j].pixel_measurement[0] << " "
            //                 << obj_meas.keypoint_measurements[j].pixel_measurement[1] << " "
            //                 << obj_meas.keypoint_measurements[j].score << "\n";
            //         }
            ...
            //     visu_save_file << "\n";
                ...
                // visu_save_file << "### Visible Objects ###\n";
                // for (size_t i = 0; i < n_visible; ++i) {
                //     visu_save_file << "obj_id: " << estimated_objects_[object_index[i]]->id() << std::endl;
                //     visu_save_file << "obj_name: " << estimated_objects_[object_index[i]]->obj_name() << std::endl;

                //     visu_save_file << "quaternion: ";
                //     for (size_t j = 0; j < 3; ++j) {
                //         visu_save_file << estimated_objects_[object_index[i]]->pose().rotation().vec()[j] << " ";
                //     }
                //     visu_save_file << estimated_objects_[object_index[i]]->pose().rotation().w() << std::endl;

                //     visu_save_file << "translation: ";
                //     for (size_t j = 0; j < 3; ++j) {
                //         visu_save_file << estimated_objects_[object_index[i]]->pose().translation()[j] << " ";
                //     }

                //     visu_save_file << "\ncoefficients: ";
                //     for (size_t j = 0; j < estimated_objects_[object_index[i]]->getCoefficients().size(); ++j) {
                //         visu_save_file << estimated_objects_[object_index[i]]->getCoefficients()(j) << " ";
                //     }
                //     visu_save_file << std::endl;

                //     visu_save_file << "3D_keypoints:\n";
                //     for (size_t j = 0; j < estimated_objects_[object_index[i]]->keypoints().size(); ++j) {
                //         visu_save_file << estimated_objects_[object_index[i]]->keypoints()[j]->position()(0) << " "
                //             << estimated_objects_[object_index[i]]->keypoints()[j]->position()(1) << " "
                //             << estimated_objects_[object_index[i]]->keypoints()[j]->position()(2) << "\n";
                //     }
                //     visu_save_file << "\n";
                // }
                // visu_save_file << "\n";

                // // save hungarian and probabilistic weights
                // visu_save_file << "### Association Weight ###\n";
                // visu_save_file << "mahalanobis distance:\n" << mahals << "\n\n";

                // visu_save_file << "cobariance determinant:\n" << covDets << "\n\n";

                // visu_save_file << "permanent weights:\n" << weights << "\n\n";    
                // visu_save_file << "hungarian weights:\n" << hungarian_weights;

            // }

            /******************************************* end saving data for association visualization ********************************************/


            // if (weights.rows() > 0) {
            //     // debug
            //     // double w = weights.array()(0, 0);
            //     // std::cout << "weights(0, 0): " << w << std::endl;
            //     // std::cout << "weights(0, 0) is nan: " << weights.array().isNaN() << std::endl;
            //     // std::cout << "weights(0, 0) is nan sum: " << weights.array().isNaN().sum() << std::endl;

            //     // std::cout << "weights(0, 0) is inf: " << weights.array().isInf() << std::endl;
            //     // std::cout << "weights(0, 0) is inf sum: " << weights.array().isInf().sum() << std::endl;

            //     if (weights.array().isNaN().sum() || weights.array().isInf().sum()) {
            //         std::cout << "encountered nan or inf" << std::endl;
            //         weights = hungarian_weights;
            //     }
            // }
            
            // if (weights.rows() > 0) {
            //     if (weights.array().isNaN() || weights.array().isInf()) {
            //         std::cout << "encountered nan or inf" << std::endl;
            //         weights = hungarian_weights;
            //     }
            // }
        ...
        // std::cout << "after weight computation" << std::endl;
        ...
        // std::cout << "constraint_weight_threshold: " << params_.constraint_weight_threshold << std::endl;
        // std::cout << "weights shape: (" << measurement_index.size() << ", " << weights.cols() - 1 << ")" << std::endl;
            ...
            for (int j = 0; j < weights.cols() - 1; ++j) {
                if (weights(k, j) >= params_.constraint_weight_threshold) {
                    ...
                    if ((frame->index() -
                          static_cast<int>(
                            estimated_objects_[object_index[j]]->last_seen()) >
                        loop_closure_threshold_) && detect_loop_closure_) {
...
SemanticMapper::updateKeyframeObjects(SemanticKeyframe::Ptr frame)
{
    ...
    // std::cout << "Weight computation time taken at time " << std::setiosflags(std::ios::fixed)<<std::setprecision(10)
    //     << ros::Time::now().toSec() << ": " << std::chrono::duration_cast<std::chrono::microseconds>(duration).count() / 1000. << " ms" << std::endl;
    // fflush(stdout);

    // if (std::chrono::duration_cast<std::chrono::microseconds>(duration).count() / 1000. > 1000) {
    //     std::cout << "Data association takes a long time." << std::endl;
    // }
  ...
  Key keyframe_key)
{
    ...
    for (auto detection : msg.detections) {
        ...
        // std::cout << "detection.obj_name: " << detection.obj_name << std::endl;
...
SemanticMapper::predictVisibleObjects(SemanticKeyframe::Ptr kf)
{
    ...
    for (size_t i = 0; i < estimated_objects_.size(); ++i) {
        ...
        Pose3 map_T_obj = estimated_objects_[i]->pose();
        Pose3 body_T_obj = map_T_body.inverse() * map_T_obj;
        Pose3 cam_T_obj = I_T_C_.inverse() * body_T_obj;

        if (cam_T_obj.translation().norm() <= 75 && cam_T_obj.translation()(2) > 0) {
            visible[i] = true;
        }
    }
