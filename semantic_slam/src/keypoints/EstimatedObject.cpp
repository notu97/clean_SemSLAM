  ...
  , mapper_(mapper)
{
    ...
    first_frame_key_ = msmt.observed_key;
...
Eigen::MatrixX2d
EstimatedObject::bbox2poly(const Eigen::Vector4d& bbox)
{
    auto xmin = bbox(0);
    auto ymin = bbox(1);
    auto xmax = bbox(2);
...
Eigen::MatrixX3d
EstimatedObject::poly2lineh(const Eigen::MatrixX2d& points)
{
...
EstimatedObject::optimizeStructure()
{
    ...
    // std::cout << "starting to add bbox measurements" << std::endl;
    // // add bbox measurements
    // for (auto obj : measurements_) {
    //     int keyframe_idx = Symbol(obj.observed_key).index();
    //     Eigen::Vector4d bbox{obj.bbox.xmin, obj.bbox.ymin, obj.bbox.xmax, obj.bbox.ymax};

    //     Eigen::MatrixX3d lines = poly2lineh(bbox2poly(bbox));
    //     for (size_t i = 0; i < 4; ++i) {
    //         Eigen::Vector3d uline_zb = lines.block<1, 3>(i, 0).transpose();
    //         structure_problem_->addBboxMeasurement(uline_zb, keyframe_idx);
    //     }        
    // }
...
EstimatedObject::computeMahalanobisDistance(const ObjectMeasurement& msmt) const
{
    ...
    // double objz = pose_.translation()[2];
    // double selfx = G_T_I.translation()[0], selfy = G_T_I.translation()[1], selfz = G_T_I.translation()[2];
    // // std::cout << "avg_objz: " << avg_objz << std::endl;
    // // std::cout << "objz: " << pose_.translation()[2] << std::endl;
    // if ( (this->last_seen() - keyframe->index() > 50) && (abs(selfz - objz) > 5)) {
    //     auto first_frame = mapper_->getKeyframeByKey(first_frame_key_);
    //     Pose3 first_selfpose = first_frame->pose();
    //     double firstx = first_selfpose.translation()[0], firsty = first_selfpose.translation()[1], firstz = first_selfpose.translation()[2];

    //     double dist_xy = sqrt(pow(firstx-selfx, 2) + pow(firsty-selfy, 2));
    //     double dist_z = abs(firstz - selfz);

    //     if (dist_xy < 10 && dist_z > 5) {
    //         G_T_I.translation()[2] = firstz;
    //     }

    //     // std::cout << "very large z difference!" << std::endl;
    //     // std::cout << "avg_objz: " << avg_objz << " selfz: " << selfz << std::endl;
    // }
    
    // // if (abs(G_T_I.translation()[2] - pose_.translation()[2]) > 5) {
    // //     std::cout << "very large z difference!" << std::endl;
    // // }
    // // std::cout << "self z: " << G_T_I.translation()[2] << std::endl;
    ...
    // double mahal = residuals.transpose() * residuals;
...
double
EstimatedObject::computeMahalanobisAndDet(const ObjectMeasurement& msmt, double& det, double avg_objz) const
{
    // if (msmt.class_id != classid_) return std::numeric_limits<double>::max();
    // ROS_WARN_STREAM("Computing mahal for object " << id());
    if (msmt.obj_name != obj_name_) {
    ...
    if (keypoints_.size() != 12){
        std::cout << "num keypoints:" << keypoints_.size() << std::endl;
    }
    ...
    for (size_t i = 0; i < msmt.keypoint_measurements.size(); ++i) {
        ...
        try {
            ...
            if (msmt.keypoint_measurements[i].pixel_sigma == 0) {
                std::cout << "pixel_sigma: " << msmt.keypoint_measurements[i].pixel_sigma << std::endl;
            }
            ...
            n_observed++;
            // std::cout << "No CheiralityException" << std::endl;
        } catch (CheiralityException& e) {
            // TODO should we ignore this or should we add a high value or
            // something
    ...
    // std::cout << "R:\n" << R << std::endl;
    // residuals = residuals.cwiseMin(-200).cwiseMax(200);
    // std::cout << "residuals:\n" << residuals << std::endl;

    double mahal = residuals.transpose() *
                   (H * Plx * H.transpose() + R).ldlt().solve(residuals);

    det = (H * Plx * H.transpose() + R).determinant();
    // std::cout << "Det: " << det << std::endl;
    // std::cout << "Det(R): " << R.determinant() << std::endl;
    if (det == 0){
        std::cout << "cov shape: (" << (H * Plx * H.transpose() + R).rows() << "," << (H * Plx * H.transpose() + R).cols() << ")\n";
        // std::cout << "R det:" << R.determinant() << std::endl;
        if (R.determinant() != 0){
            std::cout << "det(R) is not zero." << std::endl;
        }
    }
    // double mahal = residuals.transpose() * residuals;
...
EstimatedObject::removeKeypointMeasurements(const ObjectMeasurement& msmt)
{
    ...
    std::cout << "Found a measurement to remove" << std::endl;
...
EstimatedObject::addToGraph()
{
    ...
    std::cout << "start adding object to graph" << std::endl;
