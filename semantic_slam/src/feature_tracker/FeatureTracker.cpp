...
FeatureTracker::trackFeaturesForward(size_t idx1)
{
    ...
    std::string visu_save_root = "/home/hanwen/code/object_slam/semanticSLAM/estim_result/visu/geom/005/";
    uint32_t seq_id = frame2.seq;

    std::string visu_save_dir = visu_save_root + std::to_string(seq_id) + ".txt";
    std::ofstream visu_save_file(visu_save_dir);
    // save time
    visu_save_file << "time: " << frame2.stamp << "\n\n";

    for (size_t i = 0; i < frame2.feature_tracks.size(); ++i) {
        visu_save_file << frame2.feature_tracks[i].pt_id << " " << frame2.feature_tracks[i].pt.x << " " << frame2.feature_tracks[i].pt.y << "\n";
    }

    visu_save_file.close();

    /********************************** end saving geometric features *************************************/
